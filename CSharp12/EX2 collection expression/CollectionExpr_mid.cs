...
class EX2_CollectionExpr_mid : ISample
{
    public class Person(string name)
    {
        public string Name { get; set; } = name;
    }
    public class Developer(string name, int id, List<Skill> Langs) : Person(name) //<- CHANGE TYPE Array->List
    {
        public int Id => id;
        public Developer(string name, int id) : this(name, id, new() /*TARGET NEW - THERE IS NO List.Empty<Skill>() */) { }
        List<Grade> grades => Langs.Select(l => l.lvl).ToList();
